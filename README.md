...
* **索引（Index）**：不同于许多其他版本控制的工具，Git不会把一个提交里面的所有改变从工作目录树直接提交到版本仓库。而是先创建一个叫做索引（Index）的东西。读者可以想象，索引就是一个可以“确认”你的提交的地方，在你提交版本变化之前，把你修改了的文件一个一个的放进索引中。一些人喜欢用一个更加直观的词“暂存区”，而不是“索引”来称呼他。
